...
public class MovieSearchDialog extends ComponentDialog {
	private final StatePropertyAccessor<UserSearchPreferences> userProfileAccessor;
	private static final Map<String, List<String>> facets = SearchMovies.getFacets();

	public MovieSearchDialog(UserState withUserState) {
		super("UserProfileDialog");

		userProfileAccessor = withUserState.createProperty("UserProfile");

		WaterfallStep[] waterfallSteps = { MovieSearchDialog::topLevelSearchBasisStep,
				MovieSearchDialog::searchTextStep, this::searchTextConfirmStep, this::performSearchStep };
	...
	private static CompletableFuture<DialogTurnResult> topLevelSearchBasisStep(WaterfallStepContext stepContext) {
		...
		promptOptions
				.setChoices(ChoiceFactory.toChoices("Title", "Cast", "Keywords", "Genre", "Language", "Free Form"));
	...
	private CompletableFuture<DialogTurnResult> performSearchStep(WaterfallStepContext stepContext) {
		if ((Boolean) stepContext.getResult()) {
			...
			} else if (searchBasis != null && searchBasis.trim().equalsIgnoreCase("Free Form")) {
				searchBasis = "overview";
			}
				...
				return stepContext.getContext()
						.sendActivity(MessageFactory
								.text(String.format("NO MATCHES FOUND for movies having \"%s\" in the \"%s\" field ",
										searchTermFromUser, searchBasis)))
						.thenCompose(resourceResponse -> stepContext.replaceDialog("WaterfallDialog"));
			}
