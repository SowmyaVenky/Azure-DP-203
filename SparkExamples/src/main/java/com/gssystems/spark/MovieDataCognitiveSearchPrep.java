...
public class MovieDataCognitiveSearchPrep {
	...
	public static void main(String[] args) {
		...
		SparkSession spark = SparkSession.builder().appName("MovielensCognitiveSearchPrep").getOrCreate();
		...
		Dataset<Row> movie_genre_genre1 = movie_genre_genre.filter(movie_genre_genre.col("movie_id").isNotNull())
				.groupBy("movie_id").agg(org.apache.spark.sql.functions.collect_set("genre_name").as("genres"));
		...
		Dataset<Row> movie_kw2 = movie_kw1.filter(movie_kw1.col("movie_id_1").isNotNull())
				.groupBy("movie_id_1").agg(org.apache.spark.sql.functions.collect_set("keyword").as("keywords"));
		...
		if (WRITE_FILE_OUTPUTS) {
			...
			moviesdf3.repartition(1).write().mode(SaveMode.Overwrite).option("encoding", "UTF-8").json("movies_cogsearch_file");
		}
